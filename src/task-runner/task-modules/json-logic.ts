...
export class JsonLogic extends Basic<any> {
  async main(context: TaskRunnerContext, taskConfig: TaskConfig, config: any, root: JQuery<HTMLElement>): Promise<any> {
    const outcome = JsonLogicParser.apply(config, context);
    let output: any;
    if (outcome.tasks) {
      output = await TaskRunner.run(outcome.tasks, context, root, outcome.renderOutput);
    }
    if (outcome.halt) {
      output.__halt = true;
    }
